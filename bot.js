...
client.on('message', msg => {
  if (msg.content === 'قوانين') {
    msg.reply('في حال خالفت أحد القوانين أدناه حنعطيك إجازة بس على طريقتنا');
  }
});


client.on('guildMemberAdd', member => {
    let channel = member.guild.channels.find('name', 'الشات_العام');
...
.addField('     **1** ' ,' **شارك بالروم المخصص** ')
.addField('     **2** ' ,' **ممنوع القذف والسب ** ')
.setColor('#7d2dbe')
...
client.on('message', message => {
  ...
  message.channel.send("**:white_check_mark: تم ارسال الرابط برسالة خاصة**")
...
client.on('message',function(message) {
   ...
   if(message.content.startsWith(prefix + "mute")) {
       ...
       if(!muteReason) return message.channel.send('ℹ **Error:** ``حدد سبباً``');
...
if (message.content.startsWith(prefix + 'مسح')) {
 let args = message.content.split(" ").slice(1)
    ...
    if (args > 2000) return message.reply("**🛑 || يجب ان يكون عدد المسح أقل من 2000 .**").then(messages => messages.delete(5000))
